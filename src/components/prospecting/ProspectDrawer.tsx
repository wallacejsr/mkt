...
import { ApproachOptions, ApproachSetupModal } from './ApproachSetupModal';
...
interface Props {
  ...
  onOpenApproach: (prospectName: string, approach: any, meta: { source: string; channel: string }, onRegenerate: () => Promise<void>) => void;
}
...
export function ProspectDrawer({ prospectId, onClose, onImportToCRM, onRefreshProspects, onOpenApproach }: Props) {
  ...
  const [isApproachSetupOpen, setIsApproachSetupOpen] = useState(false);
  ...
  const handleGenerateApproach = async (options: ApproachOptions) => {
    if (!prospect) return;
    try {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-slate-900/40 backdrop-blur-xs flex justify-end">
      <div className="w-full max-w-xl bg-white h-full shadow-2xl flex flex-col border-l border-slate-200">
        ...
        {prospect && (
          <div className="p-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between gap-3">
            <button
              onClick={() => setIsApproachSetupOpen(true)}
      ...
      <ApproachSetupModal
        isOpen={isApproachSetupOpen}
        companyName={prospect?.companyName || ''}
        defaultSenderName={user?.name && !/^admin$/i.test(user.name) ? user.name : ''}
        defaultCommercialName={business?.name && !/^negócio principal$/i.test(business.name) ? business.name : 'AGRO BW'}
        generating={generatingApproach}
        onClose={() => setIsApproachSetupOpen(false)}
        onGenerate={handleGenerateApproach}
      />
    </div>
  );
}
